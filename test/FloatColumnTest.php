...
class FloatColumnTest extends \PHPUnit_Framework_TestCase
{
    public function testDecimal()
    {
        ...
        $this->assertSame('`foo` float(10,2) NOT NULL', $column->build());

        // unsigned
        $column = new FloatColumn('foo', 10, 2);
        $column->unsigned();
        $this->assertSame(true, $column->isUnsigned());
        $this->assertSame('`foo` float(10,2) UNSIGNED NOT NULL', $column->build());

        // unsigned + nullable
        $column->nullable();
        $this->assertSame('`foo` float(10,2) UNSIGNED', $column->build());

        // unsigned + nullable + default
        $column->default(1);
        $this->assertSame('`foo` float(10,2) UNSIGNED DEFAULT "1.00"', $column->build());

        // unsigned + nullable + default + comment
        $column->comment('foobar');
        $this->assertSame('`foo` float(10,2) UNSIGNED DEFAULT "1.00" COMMENT "foobar"', $column->build());

        // unsigned + default
        $column = new FloatColumn('foo', 10, 2);
        $column->unsigned()->default(1);
        $this->assertSame('`foo` float(10,2) UNSIGNED NOT NULL DEFAULT "1.00"', $column->build());

        // unsigned + default + comment
        $column->comment('foobar');
        $this->assertSame('`foo` float(10,2) UNSIGNED NOT NULL DEFAULT "1.00" COMMENT "foobar"', $column->build());

        // unsigned + comment
        $column = new FloatColumn('foo', 10, 2);
        $column->unsigned()->comment('foobar');
        $this->assertSame('`foo` float(10,2) UNSIGNED NOT NULL COMMENT "foobar"', $column->build());
        ...
        $this->assertSame('`foo` float(10,2)', $column->build());

        // nullable + default
        $column->default(1);
        $this->assertSame('`foo` float(10,2) DEFAULT "1.00"', $column->build());

        // nullable + default + comment
        $column->comment('foobar');
        $this->assertSame('`foo` float(10,2) DEFAULT "1.00" COMMENT "foobar"', $column->build());

        // nullable + comment
        $column = new FloatColumn('foo', 10, 2);
        $column->nullable()->comment('foobar');
        $this->assertSame('`foo` float(10,2) COMMENT "foobar"', $column->build());
        ...
        $this->assertSame('`foo` float(10,4) NOT NULL DEFAULT "1.1235"', $column->build());

        $column = new FloatColumn('foo', 10, 4);
        $column->default(1);
        $this->assertSame(true, $column->hasDefault());
        $this->assertSame(1.0, $column->getDefault());
        $this->assertSame('`foo` float(10,4) NOT NULL DEFAULT "1.0000"', $column->build());

        // default + comment
        $column->comment('foobar');
        $this->assertSame('`foo` float(10,4) NOT NULL DEFAULT "1.0000" COMMENT "foobar"', $column->build());
        ...
        $this->assertSame('`foo` float(10,2) NOT NULL COMMENT "foobar"', $column->build());
    }
        ...
        for ($precision = 1; $precision <= 65; $precision++) {
            for ($scale = 0; $scale <= 30; $scale++) {
                if ($precision > $scale) {
                    ...
                    $this->assertSame(sprintf('`foo` float(%d,%d) NOT NULL', $precision, $scale), $column->build());
                }
            }
        }
    }
